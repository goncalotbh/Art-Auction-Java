...
public class AVLTree<K extends Comparable<K>, V> extends AdvancedBSTree<K,V> {
   ...
   @Override
    ...
    /**
     * Every ancestor of the removed node is stored in the stack, which is not empty.
     * @param path - Stack of PathStep objects containing all ancestors of the inserted node
     */
    protected void reorganizeRem( Stack<PathStep<K,V>> path ) {
        while (!path.isEmpty()) {
            ...
            if(parent != null) {
                if (lastStep.isLeftChild) {
                    if (parent.getBalance() == 'E') {
                        parent.setBalance('R');
                        break;
                    } else if (parent.getBalance() == 'L') {
                        parent.setBalance('E');
                    } else {
                        if (((AVLNode<K, V>) parent.getRight()).getBalance() == 'L') {
                            rotateRight1E(parent, (AVLNode<K, V>) parent.getRight(), path);
                        } else {
                            rotateRight1R(parent, (AVLNode<K, V>) parent.getRight(), path);
                    ...
                    if (parent.getBalance() == 'E') {
                        parent.setBalance('L');
                        break;
                    } else if (parent.getBalance() == 'R') {
                        parent.setBalance('E');
                    } else {
                        if (((AVLNode<K, V>) parent.getLeft()).getBalance() == 'R') {
                            rotateLeft1L(parent, (AVLNode<K, V>) parent.getLeft(), path);
                        } else {
                            rotateLeft1E(parent, (AVLNode<K, V>) parent.getLeft(), path);
